...
{
    defineTypeNameAndDebug(forcePorousZones, 0);

    addToRunTimeSelectionTable(functionObject, forcePorousZones, dictionary);
}
}
...
Foam::wordList Foam::functionObjects::forcePorousZones::createFileNames
(
...
void Foam::functionObjects::forcePorousZones::writeFileHeader(const label i)
{
    switch (fileID(i))
    {
        case fileID::mainFile:
        {
            ...
            if (localSystem_)
            {
                file(i)
                    << tab
                    << "localforcePorousZones" << forceTypes << tab
        ...
        case fileID::binsFile:
        {
            ...
            for (label j = 0; j < nBin_; j++)
            {
                ...
                const word f("forcePorousZones" + jn + binForceTypes);
            ...
            if (localSystem_)
            {
                ...
                {
                    const word jn('(' + Foam::name(j) + ')');
                    const word f("localforcePorousZones" + jn + binForceTypes);
...
void Foam::functionObjects::forcePorousZones::initialise()
{
    if (initialised_)
    {
        return;
...
Foam::functionObjects::forcePorousZones::devTau() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::forcePorousZones::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::forcePorousZones::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::forcePorousZones::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::forcePorousZones::applyBins
(
...
void Foam::functionObjects::forcePorousZones::writeforcePorousZones()
{
    Log << type() << " " << name() << " write:" << nl
        << "    sum of forcePorousZones:" << nl
...
void Foam::functionObjects::forcePorousZones::writeBins()
{
    if (nBin_ == 1)
    {
        return;
...
Foam::functionObjects::forcePorousZones::forcePorousZones
(
    const word& name,
    const Time& runTime,
    const dictionary& dict
...
bool Foam::functionObjects::forcePorousZones::read(const dictionary& dict)
{
    ...
    cellZoneList_ = labelList(dict.lookup("cellZones"));
    ...
    if (dict.found("binData"))
    {
        ...
        if (nBin_ > 1)
        {
            ...
            // allocate storage for forcePorousZones and moments
            forAll(force_, i)
            {
...
void Foam::functionObjects::forcePorousZones::calcforcePorousZonesMoment()
{
    initialise();
    ...
    if (porosity_)
    {
        ...
        forAllConstIter(HashTable<const porosityModel*>, models, iter)
        {
            ...
            // TODO : provide cellZone names instead of labels in controlDict 
	        // instead of integrating over all cellZones, only integrating over the given cellZone labels

	        	
            forAll(cellZoneList_, i)
            {
                label zoneI = cellZoneList_[i];
...
Foam::vector Foam::functionObjects::forcePorousZones::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


Foam::vector Foam::functionObjects::forcePorousZones::momentEff() const
{
    return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
}


bool Foam::functionObjects::forcePorousZones::execute()
{
    return true;
}


bool Foam::functionObjects::forcePorousZones::write()
{
    calcforcePorousZonesMoment();

    if (Pstream::master())
    {
        logFiles::write();

        writeforcePorousZones();
